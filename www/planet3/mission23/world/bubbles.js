...
// src/scenes/planet/planet3/world/bubbles.ts 이식 — 타입만 제거, 로직 동일.
...
const EMISSIVE_INTENSITY = 0;                   // 0=발광 없음
// 모델의 정면이 카메라를 향하도록 하는 Y축 보정.
const FACE_OFFSET = Math.PI;
// 누운 원본 말풍선을 세우는 X축 회전.
const STAND_ROT = Math.PI / 2;

// bubbles: [{ id, q, r, text, good }]. GLB를 로드해야 하므로 async.
export async function createBubbles(scene, bubbles, { size, hexTopY = 0, floatHeight = 2.6 }) {
  ...
  const items = new Map(); // id -> { pivot, baseY, x, z, data }
