...
2. **인트로 처리**: mission2 방식(엔진 일관). 원본 타이틀 스테퍼는 버리고 엔진 상단 스테퍼 배너 + line 노드로 스토리 인트로 처리.
3. **컴포넌트 경계**: A안(얇은 엔진 래퍼). 미니게임이 퀴즈~피날레 전체를 자체 배경으로 소유.
...
| 담당 | 범위 |
|---|---|
| 엔진 노드(`mission03.json`) | 상단 미션 스테퍼 배너 + 인트로 스토리 대사(line 노드) |
| `HeartConnectStage` | 5원석 퀴즈 → 엔딩 영상 → 후일담 크로스페이드 → 성공 배너 화면(자체 배경 소유) |
...
**수정**
- `mission03.json` — 임시 골격(2노드) → 인트로 line 노드들 + minigame 노드
...
- `public/assets/planet4/` — 원본 에셋 17개 영문명 반입

## 엔진 노드 흐름 (`mission03.json`)

```
p4_m3_intro   (line, bannerNode) — 타이틀 화면 설명 대사
                 "다섯 공감 원석을 깨워 공감 다이아몬드를 완성하자!
                  마음과 마음이 연결될 때 하트 커넥트는 빛날 수 있어."
     ↓
p4_m3_story1  (line) — "왜일까? 공감 원석도, 공감 에너지도 모두 완성됐는데…"
     ↓
p4_m3_story2  (line) — "하트 커넥트를 복원하려면 다섯 공감 원석이 하나로
                        연결된 공감 다이아몬드가 필요해."
     ↓
p4_m3_story3  (line) — "누군가의 마음과 또 다른 마음이 연결될 때 다섯 원석이
                        깨어나. 모두 연결해 공감 다이아몬드를 완성하자."
     ↓
p4_m3_play    (minigame: heartConnect, next: null)
```

- 모든 노드 `hideFriend: true`, `noAuto: true`(mission2 관례 — 하티만 말하고 클릭으로 진행).
- `p4_m3_intro`가 `bannerNode` → 상단 스테퍼 배너가 이 노드에서만 강조.
...
```
phase: "quiz" → "video" → "epilogue" → "success"
```
...
| 퀴즈 피드백·후일담 하티 | 기본 하티.png | `hati-default.png` |
...
하트 커넥트 우주.png·설명하는 하티 로봇2.png·고민하는 하티.png·제안하는 하티.png(인트로/스토리는
엔진 하티 스프라이트 담당). 구현 중 실제 참조를 재확인해 확정한다.
...
- 브라우저 실검증: `tsc -b` 후 `#/planet/4?m=3`로 인트로→퀴즈→영상→후일담→성공→홈 전 구간을
  playwright로 실제 구동. 스테퍼가 퀴즈까지 보이고 피날레에서 가려지는지, 영상 재생·타이머
  정리(누수 없음)까지 확인. DEV 점프 파라미터로 반복 검증.

## 스코프 밖

- 에셋 최적화(후속 전체 게임 패스)
- BGM/사운드(`sound-design-paused` 중단 상태 유지)
- 원본 타이틀 스테퍼·인트로 이미지 이식(엔진이 대체)
