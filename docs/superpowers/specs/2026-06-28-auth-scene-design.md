...
- **학교**: 기본적으로 숨김(아래 4.1 참고). 여러 학교 단계에서는 폼 상단/이전 단계에 picker.
...
### 4.1 학교 선택 (다중 학교 대비, 초기엔 사실상 단일)

- mount 시 또는 chooser 진입 시 `getSchools()` 호출.
- 결과가 **1개**면 자동 선택하고 picker UI를 숨김 → 아이는 학교를 고를 일이 없음(초기 단계).
- 결과가 여러 개면 마지막 사용 학교(localStorage `hg.lastSchoolId`)를 기본 선택,
  필요 시 검색/스크롤 가능한 picker 노출.
