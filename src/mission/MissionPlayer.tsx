import { useEffect, useReducer, useRef, type MouseEvent, type PointerEvent as ReactPointerEvent } from "react";
...
interface VM {
  ...
  text: string;
  intro: boolean;
  choices: Choice[]; exploredSet: Set<number> | null;
  pick: ((i: number, c: Choice) => void) | null;
  lumi: string; hati: string; radar: string; radarPulse: boolean; bg: string;
  lumiGlow: boolean; bright: boolean; empathy: boolean;
  progress: "start" | "done";
  tapHint: string; showNext: boolean; ended: boolean;
  sparks: Spark[];
  muted: boolean;
  dragNode: boolean; dzShow: boolean;
  debug: string; debugId: string; debugCopied: boolean; // 노드 id 오버레이(디버깅용, production 제거 예정)
}
...
export default function MissionPlayer(props: { scenarioUrl: string; theme: MissionTheme; onExit: () => void }) {
  ...
  const vm = useRef<VM>({
    mode: "idle", bubbleKind: "none", text: "", intro: false,
    choices: [], exploredSet: null, pick: null,
    lumi: theme.lumiSprites.initial, hati: theme.hatiSprites.initial, radar: theme.radar.initial, radarPulse: false, bg: theme.bg.initial,
    lumiGlow: false, bright: false, empathy: false, progress: "start",
    tapHint: "", showNext: false, ended: false, sparks: [], muted: audioRef.current.muted,
    dragNode: false, dzShow: false, debug: "", debugId: "", debugCopied: false,
  }).current;

  const timers = useRef<{ typer?: number; auto?: number; finish?: () => void; resolve?: () => void }>({}).current;
  const sparkId = useRef(0);
  const dropZoneRef = useRef<HTMLDivElement>(null);
  // q4 드래그 상태(원본 view.js _enableCardDrag 이식). 한 번에 한 장만 드래그.
  const dnd = useRef<{
    active: boolean; startX: number; startY: number; scale: number; moved: number;
    card: HTMLButtonElement | null; idx: number; choice: Choice | null;
    pick?: (i: number, c: Choice) => void;
  }>({ active: false, startX: 0, startY: 0, scale: 1, moved: 0, card: null, idx: -1, choice: null }).current;

  useEffect(() => {
    let alive = true;
    const render = () => { if (alive) force(); };

    const sparkleBurst = () => {
      const glyphs = ["✨", "⭐", "💫", "🌟"];
      for (let i = 0; i < 16; i++) {
        const s: Spark = {
          id: sparkId.current++, ch: glyphs[i % glyphs.length],
          left: 480 + Math.random() * 1100, top: 180 + Math.random() * 760,
          delay: Math.random() * 0.5, size: 28 + Math.random() * 36,
        };
        vm.sparks.push(s);
        window.setTimeout(() => { vm.sparks = vm.sparks.filter((x) => x.id !== s.id); render(); }, 1300);
      }
      render();
    };
    ...
    const setSprite = (kind: "lumi" | "hati" | "radar", state?: string) => {
      ...
      if (kind === "radar") {
        ...
        vm.radar = state; vm.radarPulse = true; window.setTimeout(() => { vm.radarPulse = false; render(); }, 1100);
      }
    ...
    const updateScene = (node: MissionNode) => {
      ...
      const bg = theme.bg.byNode[node.id]; if (bg) vm.bg = bg; // 배경 교체(sparse, 지정 노드까지 유지)
    ...
    const execFx = (name: string) => {
      switch (name) {
        case "sparkle": audio.play("sparkle"); sparkleBurst(); break;
        case "signalRecover": audio.play("recover"); vm.lumi = "recovered"; vm.lumiGlow = true; sparkleBurst(); render(); break;
        case "empathyCard": audio.play("reveal"); vm.empathy = true; sparkleBurst(); render(); break;
        case "lightReturn": audio.play("fanfare"); vm.bright = true; vm.progress = "done"; vm.showNext = true; sparkleBurst(); render(); break;
        default: break;
      }
    };

    const typeInto = (txt: string, onDone: () => void) => {
      vm.text = ""; vm.mode = "typing"; render();
      let i = 0;
      window.clearInterval(timers.typer);
      timers.typer = window.setInterval(() => {
        vm.text = txt.slice(0, ++i); render();
        if (i >= txt.length) { window.clearInterval(timers.typer); onDone(); }
      }, 30);
      timers.finish = () => { window.clearInterval(timers.typer); vm.text = txt; render(); onDone(); };
    };

    const advanceLine = () => {
      window.clearTimeout(timers.auto);
      vm.tapHint = ""; vm.mode = "idle"; render();
      const r = timers.resolve; timers.resolve = undefined; if (r) r();
    };

    const view: RunnerView = {
      reset() {
        Object.assign(vm, {
          mode: "idle", bubbleKind: "none", text: "", intro: false, choices: [], exploredSet: null, pick: null,
          lumi: theme.lumiSprites.initial, hati: theme.hatiSprites.initial, radar: theme.radar.initial, radarPulse: false, bg: theme.bg.initial,
          lumiGlow: false, bright: false, empathy: false, progress: "start",
          tapHint: "", showNext: false, ended: false, sparks: [], dragNode: false, dzShow: false,
          debug: "", debugId: "", debugCopied: false,
        });
        render();
      },
      setDebug(node: MissionNode) {
        ...
        vm.debugId = node.id; vm.debugCopied = false;
        render();
      },
      execCommands(cmds: Command[] | undefined) {
        (cmds || []).forEach((c) => { if (c.cmd === "fx") execFx(theme.fx[c.value || ""] || c.value || ""); });
      },
      showLine(node, onTyped) {
        return new Promise<void>((resolve) => {
          timers.resolve = resolve;
          updateScene(node);
          vm.choices = []; vm.pick = null;
          vm.dragNode = false; vm.dzShow = false; // 라인 진입 시 드래그 dropZone 숨김
      ...
      showChoices(node, exploredSet, pick) {
        ...
        vm.pick = (idx, choice) => { if (vm.mode !== "choices") return; vm.mode = "idle"; vm.choices = []; render(); pick(idx, choice); };
        render();
        audio.play("pop");
      },
      end() { vm.mode = "end"; vm.ended = true; render(); },
    };
  ...
  useEffect(() => {
    Object.values(theme.bg.states).forEach((src) => { const im = new Image(); im.src = src; });
  }, [theme]);
  ...
  const onStageClick = () => {
    if (vm.mode === "typing") timers.finish?.();
      ...
      audio.play("tap");
      window.clearTimeout(timers.auto);
      vm.tapHint = ""; vm.mode = "idle"; force();
      const r = timers.resolve; timers.resolve = undefined; r?.();
    }
  };
  ...
  const copyDebugId = (e: MouseEvent) => {
    ...
    const id = vm.debugId;
    if (!id) return;
    const done = () => { vm.debugCopied = true; force(); window.setTimeout(() => { vm.debugCopied = false; force(); }, 1000); };
  ...
  const overDropZoneXY = (x: number, y: number) => {
    ...
    if (!dz) return false;
    const r = dz.getBoundingClientRect(); const m = 44;
  ...
  const onCardDown = (e: ReactPointerEvent<HTMLButtonElement>, idx: number, choice: Choice) => {
    ...
    dnd.active = true; dnd.moved = 0; dnd.scale = stageScale();
    dnd.startX = e.clientX; dnd.startY = e.clientY; dnd.idx = idx; dnd.choice = choice; dnd.card = card;
    card.classList.add("dragging");
    audio.play("pop");

    const move = (ev: PointerEvent) => {
      if (!dnd.active || !dnd.card) return;
      const dx = (ev.clientX - dnd.startX) / dnd.scale, dy = (ev.clientY - dnd.startY) / dnd.scale;
    ...
    const up = (ev: PointerEvent) => {
      ...
      const card2 = dnd.card; dnd.card = null;
      if (!card2) return;
      ...
      if (accept) {
        ...
        const i = dnd.idx, c = dnd.choice;
        window.setTimeout(() => { if (c) dnd.pick?.(i, c); }, 240);
      } else {
        audio.play("whoosh");
  ...
  return (
    <div id="viewport">
      <div id="stage" ref={stageRef} className={vm.bright ? "bright" : ""} onClick={onStageClick}>
        ...
        {/* 진행 스테퍼 */}
        <div id="progress">
          <div className="prog-title"><span className="star">⭐</span><span>탐험 진행도</span></div>
          <div className="stepper">
            <div className={`pnode ${m1cls}`} id="mission1">
              <div className="circle"><span className="num">1</span></div>
              <div className="label">마음 신호<br />탐색기</div>
            </div>
            <div className={`connector${vm.progress === "done" ? " filled" : ""}`} id="conn1" />
            <div className={`pnode ${m2cls}`} id="mission2">
              <div className="circle"><span className="num">2</span></div>
              <div className="label">공감 거울<br />깨우기</div>
            </div>
            <div className="connector" id="conn2" />
            <div className="pnode locked" id="mission3">
              <div className="circle"><span className="num">3</span></div>
              <div className="label">공감 없는<br />세상으로</div>
            </div>
          </div>
        </div>

        {/* 레이더 */}
        <img id="radar" className={vm.radarPulse ? "pulse" : ""} src={theme.radar.states[vm.radar]} alt="마음 신호 탐색기" />

        {/* 인트로 타이틀 배너 */}
        <div id="titleBanner" className={vm.intro ? "show" : ""}>
          <div className="tb-badge"><span className="tb-star">★</span><span className="tb-pill">미션 1</span><span className="tb-star">★</span></div>
          <div className="tb-title">마음 신호 탐색기</div>
          <div className="tb-ribbon"><span className="tb-spark">✦</span><span>친구의 마음을 찾아라!</span><span className="tb-spark">✦</span></div>
        </div>
        ...
        <div id="choices" className={vm.choices.length > 0 ? "show" : ""}>
          {vm.choices.map((c, idx) => {
            ...
            return (
              <button key={idx} className={`card${many ? " smaller" : ""}${explored ? " done" : ""}`} disabled={!!explored}
                onClick={dragCard
                  ? (e) => { e.stopPropagation(); e.preventDefault(); } // 드래그 카드: 합성 클릭 무시(선택은 pointerup에서)
                  : (e) => { e.stopPropagation(); audio.play("select"); vm.pick?.(idx, c); }}
                onPointerDown={dragCard ? (e) => onCardDown(e, idx, c) : undefined}>
                <div className="badge" style={{ background: theme.badgeColors[idx % theme.badgeColors.length] }}>{idx + 1}</div>
                <div className="icon" style={{ background: deco.bg }}>{deco.emoji}</div>
        ...
        {/* 공감 카드 (엔딩) */}
        <img id="empathyCard" className={vm.empathy ? "show" : ""} src="/assets/ui/empathy_card.png" alt="공감 카드" />
        ...
        {vm.showNext && <button id="nextBtn" onClick={(e) => { e.stopPropagation(); props.onExit(); }}>다음 미션으로 <span>➜</span></button>}

        {/* 디버그 노드 오버레이 (개발용 — production 제거 예정). 클릭 시 node id 복사 */}
        {vm.debug && (
          <div id="debug" className={vm.debugCopied ? "copied" : ""} onClick={copyDebugId} title="클릭하면 node id 복사">
            {vm.debugCopied ? `✓ 복사됨: ${vm.debugId}` : vm.debug}
          </div>
        )}

        <div id="tapHint" className={vm.tapHint ? "show" : ""}>{vm.tapHint}</div>

        <div id="fxLayer">
          {vm.sparks.map((s) => (
            <div key={s.id} className="spark" style={{ left: s.left, top: s.top, fontSize: s.size, animationDelay: `${s.delay}s` }}>{s.ch}</div>
          ))}
        </div>
      </div>

      <button id="muteBtn" className={vm.muted ? "off" : ""} onClick={toggleMute} aria-label="소리 켜기/끄기">
        {vm.muted ? "🔇" : "🔊"}
      </button>
      <button id="missionExit" className="btn ghost" onClick={props.onExit}>← 홈</button>
    </div>
  );
}
