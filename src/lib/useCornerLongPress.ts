...
// 좌상단 감지 정사각형 한 변(CSS px). 실기기(갤럭시 탭 A9+, 1280×800 @DPR 1.5)에서
// 측정해 정한 값이다 — 화면 가로 9.33인치에 1280 CSS px 이므로 약 137 CSS px/인치다.
// 100 이었을 때 두 손가락이 (78,68)·(49,125) 로 찍혀 두 번째가 상자 밖으로 떨어졌다.
// 1.85cm 짜리 상자에 성인 손가락 둘이 안 들어간다(자연스러운 간격이 세로 57px).
// 200 ≈ 3.7cm 로 여유 있게 잡는다. 넓혀도 아동 오발견 위험은 거의 없다 —
// 두 손가락 + 2초 유지 + PIN 이 모두 필요하고, 한 손가락으로는 아무리 눌러도 안 열린다.
const CORNER_PX = 200;
